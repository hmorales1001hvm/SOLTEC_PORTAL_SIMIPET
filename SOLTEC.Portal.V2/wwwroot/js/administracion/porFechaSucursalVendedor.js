$(document).ready(async function () {
    ...
    async function CargaLista() {
        ...
        await $.ajax({
            url: `${configuracionGlobal.sUrlServidorUtil}/api/Reportes/GetPorFechaSucursalVendedor`,
            method: 'POST',
            ...
            success: function (result) {
                ...
                $('#tblAcumulado').DataTable({
                    ...
                    select: true,
                    rowGroup: {
                        dataSrc: ['fecha', 'sucursal'],
                        startRender: function (rows, group, level) {
                            const suma = campo => rows.data().pluck(campo).reduce((a, b) => a + parseFloat(b || 0), 0);
                            const totalCols = 10;
                            const promedioXNota = () => {
                                const datos = rows.data();
                                const totalVentaNeta = datos.pluck('ventaNeta').reduce((a, b) => a + parseFloat(b || 0), 0);
                                const totalTcks = datos.pluck('tcksNetos').reduce((a, b) => a + parseFloat(b || 0), 0);
                                return totalTcks === 0 ? 0 : totalVentaNeta / totalTcks;
                            };
                            if (level === 0) {
                                return $('<tr/>').addClass('grupo-fecha')
                                    .append(`<td colspan="${totalCols}" style="font-weight:bold; background-color:#d0e0f0;">${group}</td>`);
                            } else if (level === 1) {
                                return $('<tr/>').addClass('grupo-sucursal')
                                    .append(`<td colspan="2" style="font-weight:bold; background-color:#f5f5f5;">${group}</td>`)
                                    .append(`<td style="background-color:#f5f5f5;"></td>`)
                                    .append(`<td style="text-align:right; font-weight:bold; background-color:#f5f5f5;">${suma('tcksNetos').toLocaleString('es-MX', { minimumFractionDigits: 0 })}</td>`)
                                    .append(`<td style="text-align:right; font-weight:bold; background-color:#f5f5f5;">${suma('totalDescuentos').toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>`)
                                    .append(`<td style="text-align:right; font-weight:bold; background-color:#f5f5f5;">${suma('ventaBaseComision').toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>`)
                                    .append(`<td style="text-align:right; font-weight:bold; background-color:#f5f5f5;">${suma('ventaConPremio').toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>`)
                                    ...
                                    .append(`<td style="text-align:right; font-weight:bold; background-color:#f5f5f5;">${promedioXNota().toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>`);
                            }
                        }
                    },
